...
class HASYDataset(Dataset):
    def __init__(self, csv_file, img_dir, transform=None):
        ...
        self.classes = sorted(self.df['symbol'].unique())
    ...
    def __getitem__(self, idx):
        ...
        label = self.class_to_idx[self.df.iloc[idx, 2]]
        if self.transform:
...
device = torch.device("cuda" if torch.cuda.is_all_available() else "cpu")
...
torch.save(model.state_dict(), "super_modellino.pth")
